...
def summary(state: dict) -> dict:
    return {"id": state["id"], "phase": state["phase"], "part": state["part"],
            ...
            "problem_file": str(Path(".interview-lab") / "sessions" / state["id"] / "candidate" / "README.md")}
...
class Lab:
    ...
    def _problem_readme_path(self, state: dict) -> Path:
        return safe_child(self.store.session(state["id"]), "candidate", "README.md")

    def _write_problem_readme(self, state: dict, pack: dict) -> None:
        suffix = "py" if state["config"]["language"] == "python" else "cpp"
        lines = [f"# {pack['title']}", "",
                 f"_Auto-generated by interview-lab from the parts revealed so far. "
                 f"This file is not graded and is not candidate code — implement in "
                 f"`solution.{suffix}`._"]
        for number, part in enumerate(pack["parts"][:state["part"]], 1):
            lines += ["", f"## Part {number}: {part['title']}", "", part["prompt"]]
        atomic_bytes(self._problem_readme_path(state), ("\n".join(lines) + "\n").encode("utf-8"))
    ...
    def start(self, session_id: str, familiar: bool) -> dict:
        with self.store.transaction(session_id) as state:
            ...
            self._write_problem_readme(state, pack)
